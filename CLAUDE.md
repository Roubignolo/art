...
- `agents/render/` — **moteur de rendu local (Pillow, zéro clé)** : préparation print **« fidélité d'abord »** (rognage bords, débruitage, accentuation, upscale Lanczos/Replicate — **n'altère PAS la teinte** ; profil `archive` opt-in pour scans abîmés non calibrés), **audit de fidélité colorimétrique** (`fidelity.py` — CIELAB/ΔE2000, verdict FIDÈLE/À REVOIR/INFIDÈLE au manifeste + badge cockpit ; cf. [`docs/restauration-politique.md`](docs/restauration-politique.md), né de l'incident Cézanne où le gray-world délavait le bleu volontaire de la nappe), **gestion couleur ICC** (`couleur.py` — `assurer_srgb` garantit le sRGB de livraison Gelato/Prodigi, conversion ICC relatif+BPC si source Adobe RGB/etc., fichiers tagués sRGB ; `rapport_gamut` = soft-proof informatif, `--profil-papier <icc>` Hahnemühle/Prodigi pour un vrai soft-proof, sinon heuristique chroma ; on ne pré-convertit PAS en CMYK — le RIP du POD mappe), mockups encadrés (moulures coupe d'onglet, marie-louise, verre, ombre), scènes lifestyle (perspective maison), comparatif tailles, **carte de provenance A6**, et `brand_assets.py` (bannière/icône/logo). CLI : `python -m agents.render --met-id <id> --out <dir> [--profil fidele|archive] [--profil-papier <icc>]`. Bascule cloud (Dynamic Mockups + fal.ai) via `/api/mockup` quand les clés sont posées.
