...
- `agents/sources/` — connecteurs CC0/domaine public : **Met** (sans clé), **Rijksmuseum** (`RIJKSMUSEUM_API_KEY`), **BHL** (`BHL_API_KEY`), **Art Institute of Chicago** (sans clé, IIIF), **Cleveland Museum of Art** (sans clé, dimensions + décès auteur exposés). IDs préfixés par offset anti-collision (Met < 10M, Rijks 10M–20M, BHL 20M+, **AIC 300M, Cleveland 400M** ; réservé Smithsonian 500M / Paris Musées 600M, à câbler — schémas à vérifier + clés). Note : `http_get_bytes` pose un Referer = origine de l'image (anti-hotlink AIC).
